		...
		int i_fps_num, int i_fps_den, int vbnum)
{
    ...
    printf("crop;%d,%d,%d,%d,%d;scaler:%d,%d,%d;pic:%d,%d\n",
            fschn_attr->crop.enable,fschn_attr->crop.top,fschn_attr->crop.left,fschn_attr->crop.width,fschn_attr->crop.height,
            fschn_attr->scaler.enable, fschn_attr->scaler.outwidth, fschn_attr->scaler.outheight,
            fschn_attr->picWidth, fschn_attr->picHeight);
	return 0;
...
int sample_res_encoder_param_default(IMPEncoderChnAttr *chnAttr, IMPEncoderProfile profile, IMPEncoderRcMode rcMode,
		resolution_size_t size, int outFrmRateNum, int outFrmRateDen, int outBitRate)
{
    int ret = 0;

	memset(chnAttr, 0, sizeof(IMPEncoderChnAttr));
    ret = IMP_Encoder_SetDefaultParam(chnAttr, profile, rcMode, size.width, size.height,
            outFrmRateNum, outFrmRateDen, outFrmRateNum * 2 / outFrmRateDen, 1,
            (rcMode == IMP_ENC_RC_MODE_FIXQP) ? 35 : -1, outBitRate);
    if (ret < 0) {
        IMP_LOG_ERR(TAG, "IMP_Encoder_SetDefaultParam(%dx%d) error !\n", size.width, size.height);
        return -1;
    }
...
static int sample_res_get_save_stream_file_name(int chn_num, char *path, IMPEncoderProfile profile)
{
	int ret = 0;
	time_t now;
	struct tm *now_tm;
    ...
    IMPPayloadType enType = (IMPEncoderEncType)(profile >> 24);
...
int sample_res_do_get_stream(int enc_chn_num, char *path, IMPEncoderProfile profile, int nr_frames)
{
	int ret, i;
	IMPEncoderStream stream;
	int stream_fd = sample_res_get_save_stream_file_name(enc_chn_num, path, profile);

	if (stream_fd < 0) {
		IMP_LOG_ERR(TAG, "sample_res_get_save_stream_file_name(%d,%s,0x%08x) failed\n", enc_chn_num, path, profile);
