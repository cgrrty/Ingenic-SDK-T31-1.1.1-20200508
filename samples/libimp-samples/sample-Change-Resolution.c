...
int main(int argc, char *argv[])
{
	...
	resolution_size_t fs_size[] = {resolution_size[RES_1080P], resolution_size[RES_VGA]};
	resolution_size_t enc_size[] = {resolution_size[RES_1080P], resolution_size[RES_VGA]};
	int i_fps_num[] = {25, 25};
	int i_fps_den[] = {1, 1};
	int vbnum[] = {2, 2};
	IMPEncoderChnAttr encchn_attr[STREAM_CNT];
	IMPEncoderProfile encchn_profile[] = {IMP_ENC_PROFILE_HEVC_MAIN, IMP_ENC_PROFILE_AVC_HIGH};
	...
	for (i = 0; i < STREAM_CNT; i++) {
		// 5.1、获取编码channel参数
		ret = sample_res_encoder_param_default(&encchn_attr[i], encchn_profile[i], IMP_ENC_RC_MODE_CBR,
