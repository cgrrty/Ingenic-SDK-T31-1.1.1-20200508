...
static int sc2335_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret = sc2335_read(sd, 0x3040, &tmp);
	/* denoise logic */
	sc2335_write(sd,0x3812,0x00);
	if (value < 0x740) { //<2
		if(tmp == 0x40){
			sc2335_write(sd,0x363c, 0x0e);
		} else if(tmp == 0x41){
			sc2335_write(sd,0x363c, 0x0f);
		}
	} else {//>=2
		sc2335_write(sd,0x363c,0x07);
	}

	sc2335_write(sd,0x3812,0x30);
