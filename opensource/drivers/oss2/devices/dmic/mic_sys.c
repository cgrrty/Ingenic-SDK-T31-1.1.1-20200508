...
long mic_ioctl(struct file *filp, unsigned int cmd, unsigned long args)
{
    struct mic_dev *mic_dev = m_mic_dev;
    enum dmic_rate rate;
...
long mic_ioctl_2(struct mic_dev *mic_dev, unsigned int cmd, unsigned long args)
{
	...
	if (NULL == mic_dev) {
		printk("error:(%s,%d), mic_dev is empty------.\n",__func__,__LINE__);
		return -1;
	}
    fdata = (struct mic_file_data *)mic_dev->fdata;
